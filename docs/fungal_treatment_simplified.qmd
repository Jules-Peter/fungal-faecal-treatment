...
## pH and Weight Changes Analysis

### pH Changes from 0 to 14 DPI

```{r ph-weight-data-preparation}
# Prepare pH and weight change data
ph_weight_data <- experiment_data %>%
  filter(!is.na(ph_14dpi) | !is.na(weight_14dpi)) %>%
  mutate(
    # Calculate weight change
    weight_change = weight_14dpi - weight_0dpi,
    weight_change_percent = (weight_change / weight_0dpi) * 100,
    
    # pH change from baseline (faeces pH) to 14 dpi
    ph_change = ph_14dpi - ph,
    
    # Flag data availability
    has_ph_data = !is.na(ph) & !is.na(ph_14dpi),
    has_weight_data = !is.na(weight_0dpi) & !is.na(weight_14dpi),
...
ph_summary <- ph_weight_data %>%
  filter(has_ph_data) %>%
  group_by(species) %>%
  summarise(
    n_ph = n(),
    baseline_ph_mean = mean(ph, na.rm = TRUE),
    baseline_ph_sd = sd(ph, na.rm = TRUE),
...
weight_summary <- ph_weight_data %>%
  filter(has_weight_data) %>%
  group_by(species) %>%
  summarise(
    n_weight = n(),
    weight_0dpi_mean = mean(weight_0dpi, na.rm = TRUE),
    weight_0dpi_sd = sd(weight_0dpi, na.rm = TRUE),
    weight_14dpi_mean = mean(weight_14dpi, na.rm = TRUE),
    weight_14dpi_sd = sd(weight_14dpi, na.rm = TRUE),
    weight_change_mean = mean(weight_change, na.rm = TRUE),
    weight_change_sd = sd(weight_change, na.rm = TRUE),
    weight_change_percent_mean = mean(weight_change_percent, na.rm = TRUE),
    weight_change_percent_sd = sd(weight_change_percent, na.rm = TRUE),
    .groups = "drop"
  ) %>%
  arrange(desc(weight_change_percent_mean))
...
if(nrow(weight_summary) > 0) {
  kable(
    weight_summary %>%
      select(species, n_weight, weight_0dpi_mean, weight_14dpi_mean, 
             weight_change_percent_mean, weight_change_percent_sd) %>%
      mutate(across(where(is.numeric), ~ round(.x, 2))),
    caption = "**Table 12.** Weight changes from 0 to 14 days post-inoculation",
    col.names = c("Species", "N", "Initial Weight (g)", "Final Weight (g)", 
                  "Mean % Change", "SD % Change")
  )
} else {
  cat("No weight change data available for analysis.\n")
}
```

```{r ph-weight-visualization, fig.cap="**Figure 3.** pH and weight changes over 14 days of fungal treatment. (A) pH changes from baseline to 14 DPI. (B) Percent weight change over treatment period.", fig.height=10}
# Create visualizations only if data exists
if(nrow(ph_summary) > 0 || nrow(weight_summary) > 0) {
  ...
  if(nrow(ph_summary) > 0) {
    ...
    p_ph <- ggplot() + 
      annotate("text", x = 1, y = 1, label = "No pH data available") +
      theme_void()
  }
  
  # Weight change visualization
  if(nrow(weight_summary) > 0) {
    p_weight <- ph_weight_data %>%
      filter(has_weight_data) %>%
      mutate(species_ordered = factor(species, 
                                      levels = weight_summary$species)) %>%
      ggplot(aes(x = species_ordered, y = weight_change_percent)) +
      
      # Individual points
      geom_jitter(width = 0.2, alpha = 0.6, size = 2, color = "gray50") +
      
      # Mean with confidence interval
      geom_pointrange(data = weight_summary %>%
                        mutate(species_ordered = factor(species, levels = weight_summary$species)),
                      aes(x = species_ordered,
                          y = weight_change_percent_mean,
                          ymin = weight_change_percent_mean - weight_change_percent_sd,
                          ymax = weight_change_percent_mean + weight_change_percent_sd),
                      size = 0.8, fatten = 3, color = "darkgreen") +
      
      # Reference line at 0 (no change)
      geom_hline(yintercept = 0, linetype = "dashed", alpha = 0.5) +
      
      # Add sample size labels
      geom_text(data = weight_summary %>%
                  mutate(species_ordered = factor(species, levels = weight_summary$species)),
                aes(x = species_ordered,
                    y = weight_change_percent_mean, 
                    label = paste0("n=", n_weight)),
                vjust = -1.5, size = 3) +
    ...
    p_weight <- ggplot() + 
      annotate("text", x = 1, y = 1, label = "No weight data available") +
      theme_void()
  }
  
  # Combine plots
  combined_ph_weight <- p_ph / p_weight
  print(combined_ph_weight)
  
} else {
  cat("Insufficient data for pH and weight change visualization.\n")
}
```

### Statistical Analysis of pH and Weight Changes

```{r ph-weight-statistical-analysis}
...
if(nrow(ph_summary) > 1) {
  ...
  if(nrow(ph_weight_data %>% filter(has_ph_data)) >= 10) {
    ph_anova <- aov(ph_change ~ species, data = ph_weight_data %>% filter(has_ph_data))
...
# Statistical tests for weight changes
if(nrow(weight_summary) > 1) {
  cat("\n**Weight Change Analysis:**\n\n")
  
  # Test if weight changes are significantly different from zero
  weight_change_test <- ph_weight_data %>%
    filter(has_weight_data) %>%
    group_by(species) %>%
    summarise(
      n = n(),
      mean_change_percent = mean(weight_change_percent, na.rm = TRUE),
      sd_change = sd(weight_change_percent, na.rm = TRUE),
      se_change = ifelse(n > 1, sd_change / sqrt(n), NA_real_),
      t_stat = ifelse(!is.na(se_change) & se_change > 0, mean_change_percent / se_change, NA_real_),
      p_value = ifelse(!is.na(t_stat) & n > 1, 2 * pt(abs(t_stat), df = n - 1, lower.tail = FALSE), NA_real_),
      significant = !is.na(p_value) & p_value < 0.05,
      .groups = "drop"
    )
  
  significant_weight_changes <- sum(weight_change_test$significant)
  
  cat("- Species showing significant weight changes:", significant_weight_changes, 
      "out of", nrow(weight_change_test), "\n")
  
  if(significant_weight_changes > 0) {
    cat("- Species with significant weight changes:\n")
    weight_change_test %>%
      filter(significant) %>%
      arrange(p_value) %>%
      mutate(
        direction = ifelse(mean_change_percent > 0, "gained", "lost"),
        p_display = ifelse(p_value < 0.001, "< 0.001", sprintf("%.3f", p_value))
      ) %>%
      select(species, mean_change_percent, direction, p_display) %>%
      kable(col.names = c("Species", "Mean % Change", "Direction", "p-value")) %>%
      print()
  }
  
  # Test for differences between species
  if(nrow(ph_weight_data %>% filter(has_weight_data)) >= 10) {
    weight_anova <- aov(weight_change_percent ~ species, 
                        data = ph_weight_data %>% filter(has_weight_data))
    weight_anova_summary <- summary(weight_anova)
    
    cat("\n**Between-species weight change comparison:**\n")
    cat("- F-statistic:", round(weight_anova_summary[[1]]$`F value`[1], 2), "\n")
    cat("- p-value:", format.pval(weight_anova_summary[[1]]$`Pr(>F)`[1]), "\n")
  }
}
```

### Key Findings - pH and Weight Changes

```{r ph-weight-key-findings}
...
if(nrow(weight_summary) > 0) {
  max_weight_loss <- weight_summary %>% slice_min(weight_change_percent_mean, n = 1)
  min_weight_loss <- weight_summary %>% slice_max(weight_change_percent_mean, n = 1)
  
  cat("2. **Weight Changes**:\n")
  cat("   - Maximum weight loss:", max_weight_loss$species,
      "(", round(max_weight_loss$weight_change_percent_mean, 1), "%)\n")
  cat("   - Minimum weight loss:", min_weight_loss$species,
      "(", round(min_weight_loss$weight_change_percent_mean, 1), "%)\n")
  cat("   - Average weight change across all species:",
      round(mean(weight_summary$weight_change_percent_mean), 1), "%\n\n")
}

cat("3. **Treatment Implications**:\n")
cat("   - pH changes indicate active microbial metabolism and substrate modification\n")
cat("   - Weight loss reflects decomposition and moisture evaporation\n")
cat("   - Species-specific patterns suggest different metabolic strategies\n")
```
