...
experiment_1_1_joined_filtered <- experiment_1_1_joined %>%
  ...
  mutate(
    ...
    # Calculate dry weights using water content
    dry_weight_0dpi = case_when(
      !is.na(weight_0dpi) & !is.na(water_content_0dpi) & water_content_0dpi < 100 ~ 
        weight_0dpi * (1 - water_content_0dpi),
      TRUE ~ NA_real_
    ),
    dry_weight_14dpi = case_when(
      !is.na(weight_14dpi) & !is.na(water_content_14dpi) & water_content_14dpi < 100 ~ 
        weight_14dpi * (1 - water_content_14dpi),
      TRUE ~ NA_real_
    ),
    # Calculate dry weight change
    dry_weight_change = case_when(
      !is.na(dry_weight_14dpi) & !is.na(dry_weight_0dpi) ~ dry_weight_14dpi - dry_weight_0dpi,
      TRUE ~ NA_real_
    ),
    # Calculate percent dry weight change
    dry_weight_percent_change = case_when(
      !is.na(dry_weight_0dpi) & !is.na(dry_weight_14dpi) & dry_weight_0dpi > 0 ~ 
        ((dry_weight_14dpi - dry_weight_0dpi) / dry_weight_0dpi) * 100,
      TRUE ~ NA_real_
    ),
    # Calculate percent weight change from 0 DPI to 14 DPI (wet weight)
...
# Keep only:
# - Controls: "No growth, no contamination", "No growth, contaminated", and "Growth, contaminated" 
...
experiment_1_1_ph_weight_ecoli <- experiment_1_1_joined_filtered %>%
  filter(
    # Keep controls with no growth/no contamination OR no growth/contaminated OR growth/contamination
    (species == "ctrl" & growth_status_14dpi %in% c("No growth, no contamination", "No growth, contaminated", "Growth, contaminated")) |
